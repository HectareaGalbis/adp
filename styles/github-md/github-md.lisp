...
(adppvt:def-symbol-ref-writer (stream tag file-path)
  (let* ((symbol-header (cond
			  ((symbol-macro-p tag)
			   (format nil "Symbol macro: ~a" (escape-characters (prin1-to-string tag))))
			  ((constantp tag)
			   (format nil "Constant: ~a" (escape-characters (prin1-to-string tag))))
			  (t (format nil "Variable: ~a" (escape-characters (prin1-to-string tag))))))
...
(adppvt:def-function-ref-writer (stream tag file-path)
  (let* ((function-header (cond
			    ((macro-function tag)
			     (format nil "Macro: ~a" (escape-characters (prin1-to-string tag))))
			    ((subtypep tag 'generic-function)
			     (format nil "Generic function: ~a" (escape-characters (prin1-to-string tag))))
			    (t (format nil "Function: ~a" (escape-characters (prin1-to-string tag))))))
...
(adppvt:def-type-ref-writer (stream tag file-path)
  (let* ((type-header (cond
			((subtypep tag 'condition)
			 (format nil "Condition: ~a" (escape-characters (prin1-to-string tag))))
			((subtypep tag 'structure-class)
			 (format nil "Struct: ~a" (escape-characters (prin1-to-string tag))))
			((subtypep tag 'class)
			 (format nil "Class: ~a" (escape-characters (prin1-to-string tag))))
			(t (format nil "Type: ~a" (escape-characters (prin1-to-string tag))))))
	 (type-anchor (convert-to-github-header-anchor type-header))
	 (*print-pprint-dispatch* adppvt:*custom-pprint-dispatch*))
    (format stream "[~s](/~a.md#~a)" (escape-characters (prin1-to-string tag)) file-path type-anchor)))
