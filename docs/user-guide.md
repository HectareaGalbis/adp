...
* [The ADP User Guide](/#<FILE {1005B043C3}>.md#the-adp-user-guide)
  * [Setting up ADP](/#<FILE {1005B043C3}>.md#setting-up-adp)
  * [Selecting a file to write in](/#<FILE {1005B043C3}>.md#selecting-a-file-to-write-in)
  * [Functions to generate the API](/#<FILE {1005B043C3}>.md#functions-to-generate-the-api)
  * [Functions to generate guides\.](/#<FILE {1005B043C3}>.md#functions-to-generate-guides)
    * [Headers](/#<FILE {1005B043C3}>.md#headers)
* [This is a header](/#<FILE {1005B043C3}>.md#this-is-a-header)
  * [This is a subheader](/#<FILE {1005B043C3}>.md#this-is-a-subheader)
    * [This is a subsubheader](/#<FILE {1005B043C3}>.md#this-is-a-subsubheader)
    * [Text](/#<FILE {1005B043C3}>.md#text)
    * [Tables](/#<FILE {1005B043C3}>.md#tables)
    * [Lists](/#<FILE {1005B043C3}>.md#lists)
    * [Text enrichment](/#<FILE {1005B043C3}>.md#text-enrichment)
    * [Images](/#<FILE {1005B043C3}>.md#images)
    * [Code blocks](/#<FILE {1005B043C3}>.md#code-blocks)
  * [Generating the documentation](/#<FILE {1005B043C3}>.md#generating-the-documentation)
  * [Cross references](/#<FILE {1005B043C3}>.md#cross-references)
    * [Header\-tags](/#<FILE {1005B043C3}>.md#header-tags)
    * [Function\-tags\, symbol\-tags and type\-tags](/#<FILE {1005B043C3}>.md#function-tags-symbol-tags-and-type-tags)
    * [Code\-tags](/#<FILE {1005B043C3}>.md#code-tags)
    * [Quoting\, commenting\, hiding and removing your code](/#<FILE {1005B043C3}>.md#quoting-commenting-hiding-and-removing-your-code)
  * [Tips and final comments](/#<FILE {1005B043C3}>.md#tips-and-final-comments)
## Setting up ADP
...
Before start using the macros to write documentation\, we need to select a file where to store it\. We can do this using the [ADP\:IN\-FILE](/#<FILE {1005932663}>.md#macro-in-file) macro\. You can always add this macro after [IN\-PACKAGE](http://www.lispworks.com/reference/HyperSpec/Body/m_in_pkg.htm)\.
...
If you want to generate documentation of this definition you only need to use the macro [ADP\:DEFUN](/#<FILE {1005932663}>.md#macro-defun) instead of [DEFUN](http://www.lispworks.com/reference/HyperSpec/Body/m_defun.htm)\.
...
Same as with [ADP\:DEFUN](/#<FILE {1005932663}>.md#macro-defun)\, every macro that defines something is redefined to print documentation\. You can see every macro here\: [API reference functions](/#<FILE {1005932663}>.md#api-reference-functions)
...
You can add headers in your documentation\. In other words\, they work as titles or subtitles\. You can this way organize your guide with different sections \(like I do in this guide\)\. The macros that add headers are [ADP\:HEADER](/#<FILE {1005932663}>.md#macro-header)\, [ADP\:SUBHEADER](/#<FILE {1005932663}>.md#macro-subheader) and [ADP\:SUBSUBHEADER](/#<FILE {1005932663}>.md#macro-subsubheader)\. They need a string as the first argument\. For example\, if I write this\:
...
When you want to add text you must use the macro [ADP\:TEXT](/#<FILE {1005932663}>.md#macro-text)\. It receives a variable number of arguments\. Each argument is evaluated at run\-time and its result is [PRINC](http://www.lispworks.com/reference/HyperSpec/Body/f_wr_pr.htm)\-ed\. Then\, all the content that has been [PRINC](http://www.lispworks.com/reference/HyperSpec/Body/f_wr_pr.htm)\-ed is concatenated into a single string and finally it is printed in the documentation file\. For example\:
...
You can add tables using the macro [ADP\:TABLE](/#<FILE {1005932663}>.md#macro-table)\. The best way to see how to use it is an example\. Imagine we want to show some data stored in some variables\.
...
Note that in the _Salary_ column we used multiple values in each cell\. Each cell can accept multiple values and they are treated as if they are in the [ADP\:TEXT](/#<FILE {1005932663}>.md#macro-text) macro\. In other words\, each element in a cell is [PRINC](http://www.lispworks.com/reference/HyperSpec/Body/f_wr_pr.htm)\-ed and the results are concatenated\.

### Lists

You can add lists with [ADP\:ITEMIZE](/#<FILE {1005932663}>.md#macro-itemize)\. For example\:
...
* Fruits\:
  * 0\.5Kg of apples
  * 6 oranges
Note that each item inside [ADP\:ITEMIZE](/#<FILE {1005932663}>.md#macro-itemize) is a list starting with `ITEM` or `ITEMIZE`\. When you use `ITEM` every object will be [PRINC](http://www.lispworks.com/reference/HyperSpec/Body/f_wr_pr.htm)\-ed and then concatenated\. In other words\, it works the same as [ADP\:TEXT](/#<FILE {1005932663}>.md#macro-text) or [ADP\:TABLE](/#<FILE {1005932663}>.md#macro-table)\. On the other hand\, when using `ITEMIZE` you are indicating that you want a sublist of items\.

### Text enrichment

Inside a [ADP\:TEXT](/#<FILE {1005932663}>.md#macro-text) form\, a `CELL` from a [ADP\:TABLE](/#<FILE {1005932663}>.md#macro-table) form and a `ITEM` form a [ADP\:ITEMIZE](/#<FILE {1005932663}>.md#macro-itemize) form\, we can enrich the text with the macros [ADP\:BOLD](/#<FILE {1005932663}>.md#macro-bold)\, [ADP\:ITALIC](/#<FILE {1005932663}>.md#macro-italic)\, [ADP\:EMPHASIS](/#<FILE {1005932663}>.md#macro-emphasis) and [ADP\:WEB\-LINK](/#<FILE {1005932663}>.md#macro-web-link)\. For example\:
...
Note that spaces are placed out of enrichment functions \(after `italic` and `web\\\-link` calls\)\. Also\, you cannot nest calls of [ADP\:BOLD](/#<FILE {1005932663}>.md#macro-bold)\, [ADP\:ITALIC](/#<FILE {1005932663}>.md#macro-italic)\, [ADP\:EMPHASIS](/#<FILE {1005932663}>.md#macro-emphasis) and [ADP\:WEB\-LINK](/#<FILE {1005932663}>.md#macro-web-link)\. For example\, if you try this\:
...
You can add images with the macro [ADP\:IMAGE](/#<FILE {1005932663}>.md#macro-image)\. For example\, an image is located at `guides\\\/images\\\/`\. If I evaluate the next expression\:
...
A good Lisp tutorial must include Lisp code examples\. ADP defines two macros to print code blocks\: [ADP\:CODE\-BLOCK](/#<FILE {1005932663}>.md#macro-code-block) and [ADP\:CODE\-EXAMPLE](/#<FILE {1005932663}>.md#macro-code-example)\. The main difference is that the former does not evaluate the code to be printed\. So\, for example\:
...
Note that `NIL` or\, equivalently\, `\\\(\\\)` is used after `code\\\-block`\. This is because we can pass some symbols to [ADP\:CODE\-BLOCK](/#<FILE {1005932663}>.md#macro-code-block) in order to change a bit how it works\. But\, we will see that in later sections\.

On the other hand\, [ADP\:CODE\-EXAMPLE](/#<FILE {1005932663}>.md#macro-code-example) do evaluate the code\. And what is more\, it prints the standard output as well as the returned values\. For example\:
...
A header\-tag is a symbol with a header associated\. We have already seen how to add a header to the documentation\. But I didn\'t say that the macros [ADP\:HEADER](/#<FILE {1005932663}>.md#macro-header)\, [ADP\:SUBHEADER](/#<FILE {1005932663}>.md#macro-subheader) and [ADP\:SUBSUBHEADER](/#<FILE {1005932663}>.md#macro-subsubheader) receives a second optional argument\. As you can imagine\, this second argument must be a symbol that will be converted to a header tag\. For example\, the first header of this file is created with this expression\:

```Lisp
(ADP:HEADER "The ADP User Guide" USER-GUIDE-HEADER)
```

Now the symbol `user\\\-guide\\\-header` is a header\-tag\. We can make a reference to that header with the macro [ADP\:HEADER\-REF](/#<FILE {1005932663}>.md#macro-header-ref)\. For example\, if I write this\:
...
Go to the top\: [The ADP User Guide](/#<FILE {1005B043C3}>.md#the-adp-user-guide)

Note that the macro is used inside a [ADP\:TEXT](/#<FILE {1005932663}>.md#macro-text) form\. Same as with [ADP\:BOLD](/#<FILE {1005932663}>.md#macro-bold) or [ADP\:ITALIC](/#<FILE {1005932663}>.md#macro-italic)\, [ADP\:HEADER\-REF](/#<FILE {1005932663}>.md#macro-header-ref) only can be used inside [ADP\:TEXT](/#<FILE {1005932663}>.md#macro-text)\, [ADP\:TABLE](/#<FILE {1005932663}>.md#macro-table) or [ADP\:ITEMIZE](/#<FILE {1005932663}>.md#macro-itemize)\.

### Function\-tags\, symbol\-tags and type\-tags

These tags are symbols associated with a function\, a variable or a type respectively\. More specifically\, the macros used to define things like [ADP\:DEFUN](/#<FILE {1005932663}>.md#macro-defun)\, [ADP\:DEFPARAMETER](/#<FILE {1005932663}>.md#macro-defparameter) or [ADP\:DEFSTRUCT](/#<FILE {1005932663}>.md#macro-defstruct) can create automatically a function\-tag\, a symbol\-tag or a type\-tag respectively\. The tag created is the symbol of the name of the function\, variable or type defined respectively\. ADP defines three types of tags because the same symbol can refer to a function\, a variable and a type simultaneously\. The next list shows what type of tags are defined by which macros\:

* Function\-tags\:
  * [ADP\:DEFGENERIC](/#<FILE {1005932663}>.md#macro-defgeneric)
  * [ADP\:DEFINE\-MODIFY\-MACRO](/#<FILE {1005932663}>.md#macro-define-modify-macro)
  * [ADP\:DEFMACRO](/#<FILE {1005932663}>.md#macro-defmacro)
  * [ADP\:DEFUN](/#<FILE {1005932663}>.md#macro-defun)
* Symbol\-tags\:
  * [ADP\:DEFCONSTANT](/#<FILE {1005932663}>.md#macro-defconstant)
  * [ADP\:DEFINE\-SYMBOL\-MACRO](/#<FILE {1005932663}>.md#macro-define-symbol-macro)
  * [ADP\:DEFPARAMETER](/#<FILE {1005932663}>.md#macro-defparameter)
  * [ADP\:DEFVAR](/#<FILE {1005932663}>.md#macro-defvar)
* Type\-tags\:
  * [ADP\:DEFCLASS](/#<FILE {1005932663}>.md#macro-defclass)
  * [ADP\:DEFINE\-CONDITION](/#<FILE {1005932663}>.md#macro-define-condition)
  * [ADP\:DEFSTRUCT](/#<FILE {1005932663}>.md#macro-defstruct)
  * [ADP\:DEFTYPE](/#<FILE {1005932663}>.md#macro-deftype)
Same as with header\-tags\, we can make reference to functions\, variables and types with [ADP\:FUNCTION\-REF](/#<FILE {1005932663}>.md#macro-function-ref)\, [ADP\:SYMBOL\-REF](/#<FILE {1005932663}>.md#macro-symbol-ref) and [ADP\:TYPE\-REF](/#<FILE {1005932663}>.md#macro-type-ref)\. For example\, to make a reference to an ADP macro\:
...
A reference to an ADP macro\: [ADP\:HEADER](/#<FILE {1005932663}>.md#macro-header)

Note again that [ADP\:FUNCTION\-REF](/#<FILE {1005932663}>.md#macro-function-ref) only can be used inside [ADP\:TEXT](/#<FILE {1005932663}>.md#macro-text)\, [ADP\:TABLE](/#<FILE {1005932663}>.md#macro-table) or [ADP\:ITEMIZE](/#<FILE {1005932663}>.md#macro-itemize)\. The same goes to [ADP\:SYMBOL\-REF](/#<FILE {1005932663}>.md#macro-symbol-ref) and [ADP\:TYPE\-REF](/#<FILE {1005932663}>.md#macro-type-ref)\.
...
In the future\, we will define the symbol [ADP\:\:A\-PARAMETER\-DEFINED\-AT\-THE\-END\-OF\-THE\-FILE](/#<FILE {1005B043C3}>.md#variable-a-parameter-defined-at-the-end-of-the-file)\.
...
But now you decide to use vectors rather than lists\. You didn\'t use code\-tags so you must change your code in two different places\. Let\'s create now a code\-tag using the macro [ADP\:CODE\-TAG](/#<FILE {1005932663}>.md#macro-code-tag)\. Unfortunately\, the macro [ADP\:CODE\-TAG](/#<FILE {1005932663}>.md#macro-code-tag) cannot be printed inside code\-block\. So\, I will use `code\\\-lag` instead\:
...
First we indicate in the list after `code\\\-block` that we will use the code tag named `sum\\\-list\\\-example`\. Then\, we use it\. Now\, each time you change the call to `sum\\\-list` in your test code the tutorial will be automatically changed\. You can specify as many tags as you want in both [ADP\:CODE\-TAG](/#<FILE {1005932663}>.md#macro-code-tag) and [ADP\:CODE\-BLOCK](/#<FILE {1005932663}>.md#macro-code-block) macros\.

Let\'s see a live example\. Do you remember the symbol [ADP\:\:A\-PARAMETER\-DEFINED\-AT\-THE\-END\-OF\-THE\-FILE](/#<FILE {1005B043C3}>.md#variable-a-parameter-defined-at-the-end-of-the-file) and the type [ALSO\-A\-TYPE\?](/#<FILE {1005B043C3}>.md#type-also-a-type)\?\. In the source file I have written this\:
...
Code tags are great but [ADP\:CODE\-TAG](/#<FILE {1005932663}>.md#macro-code-tag) will expand to the code you write\. Sometimes you will want to create a tag to some piece of code but not to evaluate it\. In that case we can quote the code using the form `code\\\-quote`\. For example\, you can write this\:
...
Comments are ignored when Lisp is reading an expression\, so you cannot place a regular comment inside a [ADP\:CODE\-BLOCK](/#<FILE {1005932663}>.md#macro-code-block) form and expect to see it printed\. If you want to print a comment you need to use the form `code\\\-comment` inside [ADP\:CODE\-TAG](/#<FILE {1005932663}>.md#macro-code-tag)\. It just receives a string\. For example\, if I write this\:
...
* ***Use a different system for documentation generation***\: I recommend to use a different system to indicate all the files you need to load to generate the documentation\. So\, if you have a system named `MY\\\-SYSTEM` then create another system named `MY\\\-SYSTEM\\\/DOCS`\. Although ADP will not execute anything unless you use the function [ADP\:LOAD\-SYSTEM](/#<FILE {1005932663}>.md#function-load-system)\, I think this should make your projects cleaner\. And\, let\'s be honest\, I\'m still learning the language and I don\'t want to break other people\'s code\. I did this separation for ADP\, so you can see an example in the file `adp\\\.asd`\.
* ***Handling error messages***\: I tried to make informative error messages but sometimes this cannot be possible\. Or\, at least\, I can\'t do it better\. The most common errors I have had when using ADP were undefined variable errors\. Remember that [ADP\:INLINE\-CODE](/#<FILE {1005932663}>.md#macro-inline-code) works the same as [ADP\:TEXT](/#<FILE {1005932663}>.md#macro-text)\. You can\'t write `\\\(inline\\\-code name\\\-of\\\-function\\\)`\, you must write this instead `\\\(inline\\\-code \\\"name\\\-of\\\-function\\\"\\\)` or `\\\(inline\\\-code \\\'name\\\-of\\\-function\\\)`\. Also\, be careful when using [ADP\:FUNCTION\-REF](/#<FILE {1005932663}>.md#macro-function-ref) or similars\. If you don\'t write correctly the macro\, some implementations will treat that call as a function call and will treat the argument as a variable\. That\'s not a variable that ADP or you have defined and it is sure that it will raise an undefined variable error\.
...
* ***Read the API***\: Maybe reading [ADP User Interface](/#<FILE {1005932663}>.md#adp-user-interface) can make you understand better how some macros work \(or not\)\. At least\, you may be interested in seeing the section [Additional functions](/#<FILE {1005932663}>.md#additional-functions)\.
* ***That\\\'s all\\\! Enjoy using ADP\\\.*** I leave you with [ADP\:\:A\-PARAMETER\-DEFINED\-AT\-THE\-END\-OF\-THE\-FILE](/#<FILE {1005B043C3}>.md#variable-a-parameter-defined-at-the-end-of-the-file) and [ALSO\-A\-TYPE\?](/#<FILE {1005B043C3}>.md#type-also-a-type) again\.
...
Go back to [Function\-tags\, symbol\-tags and type\-tags](/#<FILE {1005B043C3}>.md#function-tags-symbol-tags-and-type-tags)
